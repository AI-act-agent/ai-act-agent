...
1. 의존성 설치:
   ...
   ```
2. **벡터스토어 구축** (최초 1회, 법령 + 가이드라인 3종 통합 임베딩):
   ```bash
   python -m backend.app.rag.build_index
   # → data/processed/all_chunks.json (통합 660개) + data/vector_store/law_embeddings.json 생성
   #   (임베딩은 로컬, 키 불필요)
   ```
   > 법령 조문만 임베딩하려면: `python -c "from backend.app.rag.pipeline import build_vector_store; build_vector_store()"`
3. 프로젝트 루트에서 서버 실행:
   ```bash
